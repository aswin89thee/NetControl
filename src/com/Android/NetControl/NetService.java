...
public class NetService extends Service {
	...
	private void createFiles(List uidlist,String directory)
	{
		try{
			...
			while(iter.hasNext())
			{
				...
				File f2 = new File(storagedir+"/"+curuid+"_deny");
				if(!f.exists()){
					if(!f.createNewFile())
				...
				if(!f2.exists()){
					if(!f2.createNewFile())
						Log.d("createFiles","Unable to create deny file for "+curuid);
				}
			}
		}catch(IOException e){
	...
	public void onStart(Intent intent, int startid)
	{
		...
		startServer();
		Log.d(TAG,"Starting the nfqnltest client");
		try {
        	Runtime.getRuntime().exec("su -c \"./data/data/nfqnltest/nfqnltest\"");
        } catch (Exception e) {
        	Log.d("InitialActivity:onCreate",e.getMessage());
        	e.printStackTrace();
        }
		Log.d(TAG,"Started the nfqnltest client successfully");
	}
	...
	public int isDomainAllowed(String ip,int port,LocalSocket receiver,PrintWriter output)
	{
		...
		//Check if ip is denied for uid
		boolean denied = isIPDeniedForUID(ip,uid);
		if(denied) return 1;
	...
	//See if ip is denied for a particular uid
	boolean isIPDeniedForUID(String ip, int uid)
	{
		boolean denied = false;
		try {
			File sdDir = Environment.getExternalStorageDirectory();
			String fileString = sdDir.getAbsolutePath()+accessDomainDirectory+uid+"_deny";
			Log.d(TAG,"Inside isIPDeniedForUID. The uid directory is "+fileString);
			FileInputStream fstream = null;
			fstream = new FileInputStream(fileString);
